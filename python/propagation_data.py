...
class PropagationData:
    ...
    def load_data(cls, path, load_steps=False, load_ineqs=False, load_last_step=False):
        ...
        assert lwe_instance.is_solution(key)
        inequalities = None
        if not load_ineqs or (not load_steps and not load_last_step):
            ...
            inequalities = [
                Inequality(c, IneqType.LE if t == "<=" else IneqType.GE, b, ic, pc)
                for c, t, b, ic, pc in zip(coeffs, signs, bs, is_corrects, p_corrects)
            ]
