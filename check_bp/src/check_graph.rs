...
impl CheckGraph {
    ...
    fn add_var_nodes(&mut self, prior: HashMap<i16, f64>) -> PyResult<()> {
        ...
        for i in 0..K {
            ...
            n.set_prior(&prior_msg)
                .map_err(|e| CheckGraphError::from_bp(e))?;
            self.g.add_node(i.to_string(), Box::new(n), true, Some(prior_msg));
            self.var_nodes += 1;
        }
        Ok(())
    }
    fn add_var_node(&mut self, name: String, prior: HashMap<i16, f64>) -> PyResult<usize> {
        ...
        n.set_prior(&prior_msg)
            .map_err(|e| CheckGraphError::from_bp(e))?;
        let log_prior = prior_msg.get_log();
        let idx = self.g.add_node(name, Box::new(n), true, Some(log_prior));
        self.var_nodes += 1;
        Ok(idx)
    }
    fn add_inequality(
        ...
        if let Some(random_var_node_prior) = self.g.var_nodes[0].get_prior() {
            let zero_pdf = CheckMsg::make_zero_pdf(random_var_node_prior);
            let idx = self.g.add_node(name, Box::new(check_node), false, zero_pdf);
    ...
    fn add_inequality_prob(
        ...
        let idx = self.g.add_node(name, Box::new(check_node), false, None);
        for n in 0..self.var_nodes {
            self.g
                .add_edge(n, idx)
                .map_err(|e| CheckGraphError::from_bp(e))?;
