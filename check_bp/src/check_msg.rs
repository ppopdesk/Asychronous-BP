...
impl<const ETA: usize> CheckMsg<ETA> {
    ...
    pub fn make_zero_pdf(self) -> Option<CheckMsg<ETA>> {
        let mut zero_pdf = CheckMsg::new();
        for (v,_p) in self {
            zero_pdf[v] = 0.0;
        }
        Some(zero_pdf)
...
impl<const ETA: usize> Msg<i16> for CheckMsg<ETA> {
    ...
    fn get_log(&self) -> CheckMsg<ETA> {
        let mut new_msg: CheckMsg<ETA> = CheckMsg::new();
        for (v,p) in self {
            new_msg[v] = log(p);
        }
        new_msg
    ...
    fn add(&self, second: &Self) -> CheckMsg<ETA> {
        let mut sum: CheckMsg<ETA> = CheckMsg::new();
        for (v,p) in self {
            sum[v] += p+second[v];
        }
        sum
    ...
    fn subtract(self, mut second : CheckMsg<ETA>) -> CheckMsg<ETA> {
        for (v,p) in self {
            second[v] = p-second[v];
        }
        second
    }
    fn exponent(self) -> CheckMsg<ETA> {
        let mut new_msg : CheckMsg<ETA>  = CheckMsg::new();
        for (v,p) in self {
            new_msg[v] = exp(p);
        }
