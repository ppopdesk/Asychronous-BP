...
@click.option("--load-last/--no-load-last", default=False)
...
@click.option("--sign", default=None, type = str) #either input as ge or le
@click.option("--ineq-always-correct", default=None, type = str) #either input as ge or le
@click.option("--offset", default=0, type=int) #input is integer and gives how much b should be offset
@click.option("--p-correct", default=1.0, type=float)
@click.option("--multiple-runs", default=1, type=int)
@click.option("--certain-correct", default=0, type=int)
def new(
    ctx,
    number_inequalities,
    max_delta_v,
    ...
    for i in range(n):
        ...
        propagation_data = sample_inequalities(
            ...
            default_sign = sign,
            ineq_always_correct = ineq_always_correct,
            offset = offset,
        )
        success, propagation_data = run_with_inequalities(
            propagation_data,
